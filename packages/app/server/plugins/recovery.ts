...
export default defineNitroPlugin(async () => {
  ...
  if (stuckCreating.length > 0) {
    console.log(`[recovery] Marking ${stuckCreating.length} stuck creating project(s) as error`);
    await db
      .update(projects)
  ...
  if (stuckTransitioning.length > 0) {
    console.log(`[recovery] Resetting ${stuckTransitioning.length} stuck project(s) to stopped`);
